    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {isLoading ? (
          ...
          <div className="bg-white rounded-xl border p-8 text-center text-gray-400">No FAQs yet</div>
        ) : data?.data?.map((f) => (
          <div key={f._id} className="bg-white rounded-xl border shadow-sm overflow-hidden">
            <div
              className="flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-gray-50"
              onClick={() => setExpanded(expanded === f._id ? null : f._id)}
            >
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <span className="text-xs text-gray-400 font-mono w-6">#{f.sortOrder}</span>
                <p className="font-medium text-gray-900 text-sm truncate">{f.question}</p>
              ...
              <div className="flex items-center gap-1 ml-3">
                ...
                {expanded === f._id ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
              </div>
            </div>
            {expanded === f._id && (
