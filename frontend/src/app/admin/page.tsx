...
export default function AdminDashboard() {
  ...
  const cards = [
    ...
    {
      label: 'Categories',
      value: counts?.totalCategories ?? 0,
      icon: Tag,
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-slate-900 to-blue-900 rounded-2xl p-6 text-white relative overflow-hidden">
        ...
        <h2 className="text-2xl font-bold mb-1">OptiVision Dashboard</h2>
        <p className="text-slate-400 text-sm">Here's what's happening in your store today.</p>
      </div>

      {/* Stat cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {cards.map(({ label, value, icon: Icon, bg, text, shadow, gradient }) => (
          <div key={label} className={`bg-white rounded-2xl p-5 shadow-sm border border-slate-100 hover:shadow-md transition-shadow`}>
            <div className={`w-11 h-11 rounded-xl ${bg} flex items-center justify-center mb-4 shadow-lg ${shadow}`}>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <div>
            <h2 className="text-base font-semibold text-slate-800">Recent Inquiries</h2>
            <p className="text-xs text-slate-400 mt-0.5">Latest customer inquiries</p>
          </div>
          <a href="/admin/inquiries" className="text-xs text-blue-600 hover:text-blue-700 font-medium">
            View all →
          </a>
        </div>
