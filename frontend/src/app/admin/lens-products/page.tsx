...
interface LensProduct { _id: string; brandId: string; name: string; description: string; price: number; active: boolean; sortOrder: number; }
interface FormState { name: string; description: string; price: string; active: boolean; sortOrder: string; }

const EMPTY: FormState = { name: '', description: '', price: '', active: true, sortOrder: '0' };

export default function LensProductsPage() {
  const qc = useQueryClient();
  const [expandedBrand, setExpandedBrand] = useState<string | null>(null);
  const [editing, setEditing]   = useState<LensProduct | null>(null);
  const [adding, setAdding]     = useState<string | null>(null); // brandId
  ...
  function openEdit(p: LensProduct) {
    setAdding(null);
    setEditing(p);
    setForm({ name: p.name, description: p.description, price: String(p.price), active: p.active, sortOrder: String(p.sortOrder) });
    setExpandedBrand(p.brandId);
  }

  function submitAdd(brandId: string) {
    if (!form.name || !form.price) return toast.error('Name and price are required');
    createMutation.mutate({ brandId, name: form.name, description: form.description, price: Number(form.price), active: form.active, sortOrder: Number(form.sortOrder) });
  }

  function submitEdit() {
    if (!editing) return;
    if (!form.name || !form.price) return toast.error('Name and price are required');
    updateMutation.mutate({ id: editing._id, body: { name: form.name, description: form.description, price: Number(form.price), active: form.active, sortOrder: Number(form.sortOrder) } });
  }
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4 space-y-6">
      ...
      {brands.map(brand => {
        const products = allProducts.filter(p => p.brandId === brand._id);
          ...
          <div key={brand._id} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <Button size="sm" onClick={e => { e.stopPropagation(); openAdd(brand._id); }}
                  className="text-white text-xs" style={{ background: 'var(--theme-primary, #2563eb)' }}>
                          ...
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                ...
                                {!p.active && <span className="text-[10px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded font-bold">Inactive</span>}
                              </div>
                            ...
                            <div className="flex items-center gap-1 shrink-0">
                              ...
                              <button onClick={() => { if (confirm(`Delete "${p.name}"?`)) deleteMutation.mutate(p._id); }}
...
function ProductForm({ form, setForm, onSave, onCancel, saving, label }: {
  form: FormState;
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="sm:col-span-2 space-y-1">
          ...
          <Input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. Single Vision Basic" />
        </div>
        <div className="space-y-1">
          <Label className="text-xs font-bold">Price (₹) *</Label>
          <Input type="number" value={form.price} onChange={e => setForm({ ...form, price: e.target.value })} placeholder="e.g. 2500" min={0} />
        </div>
      </div>
      <div className="space-y-1">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={onSave} disabled={saving}
            className="text-white" style={{ background: 'var(--theme-primary, #2563eb)' }}>
            {saving ? 'Saving…' : label}
          </Button>
        </div>
      </div>
