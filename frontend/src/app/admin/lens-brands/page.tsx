...
export default function LensBrandsPage() {
  ...
  const lensBrands = data?.data ?? [];

  return (
    <div className="space-y-4 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <h1 className="text-xl md:text-2xl font-bold text-slate-800">Lens Brands</h1>
        <Button onClick={openCreate} size="sm">
          <Plus className="w-4 h-4 md:mr-2" /><span className="hidden md:inline">Add Lens Brand</span>
        </Button>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-slate-50 md:hidden">
          {isLoading ? (
            ...
            <div className="py-10 text-center text-slate-400 text-sm">No lens brands yet</div>
          ) : lensBrands.map((b) => (
            <div key={b._id} className="flex items-center gap-3 px-4 py-3">
              ...
              <div className="flex gap-1 shrink-0">
                ...
                <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { if (confirm('Delete?')) remove.mutate(b._id, { onSuccess: () => toast.success('Deleted') }); }}><Trash2 className="w-3.5 h-3.5 text-red-500" /></Button>
              </div>
            </div>
          ))}
        </div>
            ...
            <tbody className="divide-y divide-slate-50">
              {isLoading ? (
                ...
                <tr key={b._id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-4 py-3">{b.logo ? <Image src={b.logo} alt={b.name} width={48} height={32} className="object-contain" /> : <div className="w-12 h-8 bg-slate-100 rounded-lg" />}</td>
