...
export default function AdminCouponsPage() {
  ...
  async function doVerify(id: string) {
    ...
    try {
      ...
      toast.error((err as { response?: { data?: { message?: string } } })?.response?.data?.message ?? 'Not found');
    } finally { setVerifying(false); }
  }

  function handleScan(scanned: string) { setScannerOpen(false); setVerifyId(scanned); doVerify(scanned); }
    ...
    <div className="space-y-4 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5 hidden sm:block">Manage coupon codes — tap a row to see who claimed it</p>
        </div>
        <Button onClick={openCreate} size="sm" className="gap-1.5">
          <Plus className="w-4 h-4" /><span className="hidden md:inline">New Coupon</span>
        </Button>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 md:p-5">
        <h2 className="font-semibold text-slate-800 mb-3 flex items-center gap-2 text-sm md:text-base">
          <ScanLine className="w-4 h-4 text-blue-600 shrink-0" /> Verify Coupon at Store
        </h2>
        <form onSubmit={(e) => { e.preventDefault(); doVerify(verifyId); }} className="flex gap-2">
          <Input
            value={verifyId}
            onChange={e => setVerifyId(e.target.value.toUpperCase())}
            placeholder="Enter Claim ID…"
            className="font-mono flex-1 min-w-0 text-sm"
          />
          {hasCamera && (
            <Button type="button" variant="outline" size="icon" className="shrink-0" onClick={() => setScannerOpen(true)} title="Scan QR">
              <Camera className="w-4 h-4" />
            </Button>
          )}
          <Button type="submit" size="sm" className="shrink-0" disabled={verifying || !verifyId.trim()}>
            {verifying ? '…' : 'Verify'}
          </Button>
        </form>
        <p className="text-xs text-slate-400 mt-2">e.g. CLM-A1B2C3D4</p>

        {verifyResult && (
          <div className={`mt-3 rounded-xl p-3.5 ${verifyResult.alreadyRedeemed ? 'bg-red-50 border border-red-200' : 'bg-green-50 border border-green-200'}`}>
            {verifyResult.alreadyRedeemed ? (
              <div className="flex items-start gap-2">
                <XCircle className="w-5 h-5 text-red-500 shrink-0 mt-0.5" />
                <div>
                  <p className="font-semibold text-red-700 text-sm">Already Redeemed</p>
                  <p className="text-red-600 text-xs mt-0.5">
                    Used by <strong>{verifyResult.data.name}</strong> ({verifyResult.data.phone})
                    {verifyResult.data.redeemedAt && ` on ${new Date(verifyResult.data.redeemedAt).toLocaleString('en-IN')}`}
                  </p>
                </div>
              </div>
            ) : (
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="font-semibold text-green-700 text-sm">Valid — Marked as Redeemed</p>
                  <p className="text-green-600 text-xs mt-0.5">
                    <strong>{verifyResult.data.couponTitle}</strong> ({verifyResult.data.couponCode})
                  </p>
                  <p className="text-green-600 text-xs">
                    Customer: <strong>{verifyResult.data.name}</strong> · {verifyResult.data.phone}
                  </p>
                </div>
              </div>
            )}
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            <code className="bg-slate-100 text-slate-800 px-2 py-0.5 rounded text-xs font-mono font-semibold">{c.code}</code>
                            <Badge className={`${tc.className} text-xs`}>{tc.label}</Badge>
                            <Badge className={c.active ? 'bg-green-100 text-green-700 border-0 text-xs' : 'bg-slate-100 text-slate-500 border-0 text-xs'}>
                              {c.active ? 'Active' : 'Inactive'}
                            </Badge>
                          </div>
                          <p className="font-medium text-slate-800 text-sm mt-1 truncate">{c.title}</p>
                          <div className="flex items-center gap-2 mt-1">
                            <Users className="w-3 h-3 text-slate-400" />
                            <span className="text-xs text-slate-500">{c.usedCount}/{c.maxUses} used</span>
                            {c.validUntil && (
                              <span className="text-xs text-slate-400">· Until {new Date(c.validUntil).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                            )}
                          </div>
                        </div>
                        <div className="flex gap-1 shrink-0">
                          <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-blue-500" title="Give to customer"
                            onClick={() => { setGiveOpen(c); setGiveName(''); setGivePhone(''); }}>
                          ...
                          <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-red-400" onClick={() => handleDelete(c._id)}><Trash2 className="w-3.5 h-3.5" /></Button>
                        </div>
                      </div>
                    </div>
                    {isExpanded && (
                      <div className="bg-slate-50/80 px-4 py-3 border-t border-slate-100">
                        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Claims ({claims.length})</p>
                        {claims.length === 0 ? (
                          ...
                          <div className="space-y-2">
                            {claims.map((cl, i) => (
                              <div key={i} className="bg-white rounded-lg border border-slate-200 px-3 py-2 text-sm">
                                ...
                                <p className="text-slate-500 text-xs">{cl.phone}</p>
                                <p className="text-slate-400 text-xs font-mono">{cl.claimId}</p>
                              </div>
                            ))}
                          </div>
                        )}
                        ...
                        <tr className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-right">
                            <div className="flex items-center justify-end gap-1">
                              <Button size="sm" variant="ghost" title="Give to customer" onClick={() => { setGiveOpen(c); setGiveName(''); setGivePhone(''); }} className="h-8 w-8 p-0 text-blue-500 hover:bg-blue-50">
                                <UserPlus className="w-3.5 h-3.5" />
                              </Button>
                              <Button size="sm" variant="ghost" onClick={() => setExpanded(isExpanded ? null : c._id)} className="h-8 w-8 p-0 text-slate-500">
                                {isExpanded ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                              </Button>
                              <Button size="sm" variant="ghost" onClick={() => openEdit(c)} className="h-8 w-8 p-0"><Pencil className="w-3.5 h-3.5" /></Button>
                              <Button size="sm" variant="ghost" onClick={() => handleDelete(c._id)} className="h-8 w-8 p-0 text-red-400 hover:bg-red-50"><Trash2 className="w-3.5 h-3.5" /></Button>
                            </div>
                          </td>
                        </tr>
                        {isExpanded && (
                          <tr key={`${c._id}-claims`} className="bg-slate-50/80 border-b border-slate-100">
                            <td colSpan={7} className="px-6 py-4">
                              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Claims ({claims.length})</p>
                              {claims.length === 0 ? (
                                <p className="text-slate-400 text-sm">No one has claimed this coupon yet.</p>
                              ) : (
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="w-[calc(100vw-1.5rem)] sm:max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-2">
            ...
            {/* Banner image */}
            <div className="space-y-1.5">
              <Label>Banner Image <span className="text-slate-400 font-normal">(optional)</span></Label>
              ...
              {(bannerFile || form.bannerImage) && (
                <Button type="button" variant="outline" size="sm" className="w-full text-slate-500"
                  onClick={() => { setBannerFile(null); setForm(f => ({ ...f, bannerImage: '' })); }}>
            ...
            {/* Background color */}
            <div className="space-y-1.5">
              <Label>Card Background Color <span className="text-slate-400 font-normal">(optional)</span></Label>
              <div className="flex items-center gap-3 flex-wrap">
                <input type="color" value={form.bgColor || '#f1f5f9'} onChange={e => setForm(f => ({ ...f, bgColor: e.target.value }))}
      ...
      <Dialog open={!!giveOpen} onOpenChange={() => setGiveOpen(null)}>
        <DialogContent className="w-[calc(100vw-1.5rem)] sm:max-w-sm">
          ...
          <p className="text-sm text-slate-500 -mt-1">
            Bypasses the one-per-phone limit. Manually giving <span className="font-semibold text-slate-700">{giveOpen?.code}</span>.
          </p>
